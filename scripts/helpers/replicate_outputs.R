...
write_inferential_method_overview <- function(analysis_methods, output_dir) {
    ...
    lines <- c(
        ...
        "- Barplot whiskers are +/- 1 standard error for each plotted group mean on that linear method-specific fold-change scale, so both control and treatment bars show uncertainty when biological replicates are available.",
        "- For `normalized_t_test`, `normalized_signal` means one sample/analyte value computed as averaged duplicate raw analyte signal divided by that sample's reference-spot denominator. The denominator uses preferred Reference Spots pairs `A1,2` and `J1,2` when present, otherwise available Reference Spots rows from that sample. The per-sample denominator, reference rows, and raw reference signals are reported in `input_qc/reference_spot_qc.tsv`.",
        "- For `normalized_t_test`, the control bar is `1`; the treatment bar is `mean(treatment-arm normalized_signal values) / mean(control-arm normalized_signal values)`; control whiskers are `1 +/- SE(control-arm normalized_signal values) / mean(control-arm normalized_signal values)`; treatment whiskers are `treatment fold change +/- SE(treatment-arm normalized_signal values) / mean(control-arm normalized_signal values)`.",
        "- For `raw_log2_lm`, bars are based on log2-scale group means and converted to the linear ratio scale: the control whisker is `2^(+/- SE(control log2 values))`, and the treatment whisker is `2^(effect_estimate_log2 +/- SE(treatment log2 values))`, so those whiskers can appear asymmetric on the linear ratio axis.",
        "- Barplot whiskers are visual group-mean SE bars on the plotted fold-change scale, not confidence intervals for the treatment-vs-control contrast.",
        ...
        "- Barplot y-axis limits are fixed across every page within a barplot set and use the smallest rounded range that includes all bars, SE whiskers, and bracket annotations in that set.",
        "",
        "Important:",
        ...
        "- Waterfall whiskers are +/- 1 standard error for each plotted method-specific effect estimate.",
        "- `normalized_t_test` uses raw-data `normalized_signal` values and reports ratio-scale fold change only when both group means are positive.",
